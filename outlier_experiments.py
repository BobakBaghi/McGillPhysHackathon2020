...
from keras2pytorch_dataset import trainset_pytorch, testset_pytorch, trainset_weights_pytorch
...
def train_pytorch_weighted(trainloader, model, criterion, optimizer, epochs):
    # train the model
    model.train()
    top1 = AverageMeter()
    losses = AverageMeter()
    for epoch in range(epochs):
        for batch_idx, (inputs, targets, weights) in enumerate(trainloader):
            ...
            loss = criterion(outputs, targets).mul(weights).sum() / weights.sum()
...
def _rew_ens_transformations_pytorch_experiment(x_train, y_train, dataset_name, single_class_ind, gpu_q, p, m=5):
    gpu_to_use = gpu_q.get()
    np.random.seed(0)
    torch.manual_seed(0)
    ...
    if SCORE_MODE == 'pl_mean':
        ...
        raise NotImplementedError
    res_file_name = '{}_transformations-{}-outlier-{}-epoch-{}-mode-{}-ens_rew_baseline_{}_{}.npz'.format(dataset_name,
                                                                                                 OP_TYPE, p,
                                                                                                 epochs, SCORE_MODE,
    ...
    #  ----------------------------------re-weighting+ensemble--------------------------------------------------
    weights = scores - scores.min()
    weights = weights / weights.max()
    original_preds = np.zeros((transformer.n_transforms, len(x_train_task), transformer.n_transforms))
    ss_weights = np.matlib.repmat(weights.reshape(-1, 1), 1, transformer.n_transforms).reshape((-1,))

    del model, optimizer, trainloader, trainset
    trainset = trainset_weights_pytorch(train_data=x_train_task_transformed, train_labels=transformations_inds,
                                        weights=ss_weights, transform=transform_train)
    weighted_criterion = nn.CrossEntropyLoss(reduction='none')


    for enIdx in range(m):
        torch.manual_seed(enIdx+1000)
        model = WideResNet(num_classes=transformer.n_transforms, depth=n, widen_factor=k, in_channel=n_channels)
        ...
        # train_pytorch(trainloader, model, criterion, optimizer, epochs)
        train_pytorch_weighted(trainloader, model, weighted_criterion, optimizer, epochs)
        ...
        del model, trainloader, optimizer
    ...
    res_file_name = '{}_transformations-{}-outlier-{}-epoch-{}-mode-{}-ens_rew_{}_{}.npz'.format(dataset_name, OP_TYPE, p, epochs, SCORE_MODE,
                                                               get_class_name_from_index(single_class_ind, dataset_name),
                                                               datetime.now().strftime('%Y-%m-%d-%H%M'))
    res_file_path = os.path.join(RESULTS_DIR, dataset_name, res_file_name)
    os.makedirs(os.path.join(RESULTS_DIR, dataset_name), exist_ok=True)
...
def _ensemble_transformations_pytorch_experiment(x_train, y_train, dataset_name, single_class_ind, gpu_q, p, m=7):
    gpu_to_use = gpu_q.get()
    np.random.seed(0)
    torch.manual_seed(0)
    torch.backends.cudnn.benchmark = True
    ...
    for enIdx in range(1, m+1):
        ...
        if enIdx % 1 == 0:
            ...
            for t in range(transformer.n_transforms):
                for s in range(len(x_train_task)):
                    preds[s, t] = neg_entropy(original_preds[t, s, :] / enIdx)

            scores = preds.mean(axis=-1)

            res_file_name = '{}_transformations-{}-outlier-{}-epoch-{}-mode-{}-ensNum-{}_{}_{}.npz'.format(
                dataset_name, OP_TYPE, p, epochs, SCORE_MODE, enIdx,
                get_class_name_from_index(single_class_ind, dataset_name),
                datetime.now().strftime('%Y-%m-%d-%H%M'))
            res_file_path = os.path.join(RESULTS_DIR, dataset_name, res_file_name)
            os.makedirs(os.path.join(RESULTS_DIR, dataset_name), exist_ok=True)
...
def _weighted_transformations_pytorch_experiment(x_train, y_train, dataset_name, single_class_ind, gpu_q, p):
    gpu_to_use = gpu_q.get()
    np.random.seed(0)
    torch.manual_seed(0)
    torch.backends.cudnn.benchmark = True
    ...
    criterion = nn.CrossEntropyLoss()
    model = WideResNet(num_classes=transformer.n_transforms, depth=n, widen_factor=k, in_channel=n_channels)
    model_bk = WideResNet(num_classes=transformer.n_transforms, depth=n, widen_factor=k, in_channel=n_channels)
    model_bk.load_state_dict(model.state_dict())
    ...
    x_train_task = x_train
    transformations_inds = np.tile(np.arange(transformer.n_transforms), len(x_train_task))
    original_weights = np.ones_like(transformations_inds)
    x_train_task_transformed = transformer.transform_batch(np.repeat(x_train_task, transformer.n_transforms, axis=0),
                                                           transformations_inds)
    trainset = trainset_pytorch(train_data=x_train_task_transformed, train_labels=transformations_inds,
                                transform=transform_train)
    ...
    if SCORE_MODE == 'pl_mean':
        ...
        raise NotImplementedError
    res_file_name = '{}_transformations-{}-outlier-{}-epoch-{}-mode-{}_reboot_0_baseline_{}_{}.npz'.format(
        dataset_name, OP_TYPE, p, epochs, SCORE_MODE,
        get_class_name_from_index(single_class_ind, dataset_name),
        datetime.now().strftime('%Y-%m-%d-%H%M'))
    res_file_path = os.path.join(RESULTS_DIR, dataset_name, res_file_name)
    os.makedirs(os.path.join(RESULTS_DIR, dataset_name), exist_ok=True)
    ...
    del model, optimizer, trainloader, trainset, scores
    trainset = trainset_weights_pytorch(train_data=x_train_task_transformed, train_labels=transformations_inds, weights=ss_weights, transform=transform_train)
    trainloader = data.DataLoader(trainset, batch_size=batch_size, shuffle=True)
    model = WideResNet(num_classes=transformer.n_transforms, depth=n, widen_factor=k, in_channel=n_channels)
    model.load_state_dict(model_bk.state_dict())
    model = torch.nn.DataParallel(model).cuda()
    weighted_criterion = nn.CrossEntropyLoss(reduction='none')
    if dataset_name == 'fashion-mnist' or dataset_name == 'mnist':
        print('SGD')
    ...
    res_file_name = '{}_transformations-{}-outlier-{}-epoch-{}-mode-{}_reboot_1_{}_{}.npz'.format(
        dataset_name, OP_TYPE, p, epochs, SCORE_MODE,
        get_class_name_from_index(single_class_ind, dataset_name),
        datetime.now().strftime('%Y-%m-%d-%H%M'))
    res_file_path = os.path.join(RESULTS_DIR, dataset_name, res_file_name)
    ...
    res_file_name = '{}_transformations-{}-outlier-{}-epoch-{}-mode-{}_reboot_2_{}_{}.npz'.format(
        dataset_name, OP_TYPE, p, epochs, SCORE_MODE,
        get_class_name_from_index(single_class_ind, dataset_name),
        datetime.now().strftime('%Y-%m-%d-%H%M'))
    res_file_path = os.path.join(RESULTS_DIR, dataset_name, res_file_name)
    ...
    res_file_name = '{}_transformations-{}-outlier-{}-epoch-{}-mode-{}_reboot_3_{}_{}.npz'.format(
        dataset_name, OP_TYPE, p, epochs, SCORE_MODE,
        get_class_name_from_index(single_class_ind, dataset_name),
        datetime.now().strftime('%Y-%m-%d-%H%M'))
    res_file_path = os.path.join(RESULTS_DIR, dataset_name, res_file_name)
    ...
    res_file_name = '{}_transformations-{}-outlier-{}-epoch-{}-mode-{}_reboot_4_{}_{}.npz'.format(
        dataset_name, OP_TYPE, p, epochs, SCORE_MODE,
        get_class_name_from_index(single_class_ind, dataset_name),
        datetime.now().strftime('%Y-%m-%d-%H%M'))
    res_file_path = os.path.join(RESULTS_DIR, dataset_name, res_file_name)
    ...
    res_file_name = '{}_transformations-{}-outlier-{}-epoch-{}-mode-{}_reboot_5_{}_{}.npz'.format(
        dataset_name, OP_TYPE, p, epochs, SCORE_MODE,
        get_class_name_from_index(single_class_ind, dataset_name),
        datetime.now().strftime('%Y-%m-%d-%H%M'))
    res_file_path = os.path.join(RESULTS_DIR, dataset_name, res_file_name)
...
def _iterated_weighted_transformations_pytorch_experiment(x_train, y_train, dataset_name, single_class_ind, gpu_q, p):
    gpu_to_use = gpu_q.get()
    np.random.seed(0)
    torch.manual_seed(0)
    torch.backends.cudnn.benchmark = True
    ...
    #  ----------------------------------use iterated weighted loss---------------------------------------------------
    ss_weights = np.ones_like(transformations_inds)
    ...
    for i_epoch in range(epochs):
        trainset = trainset_weights_pytorch(train_data=x_train_task_transformed, train_labels=transformations_inds,
                                            weights=ss_weights, transform=transform_train)
        trainloader = data.DataLoader(trainset, batch_size=batch_size, shuffle=True)
        train_pytorch_weighted(trainloader, model, weighted_criterion, optimizer, epochs=1)
    ...
    res_file_name = '{}_transformations-{}-outlier-{}-epoch-{}-mode-{}_iter_weighted_{}_{}.npz'.format(
        dataset_name, OP_TYPE, p, epochs, SCORE_MODE,
        get_class_name_from_index(single_class_ind, dataset_name),
        datetime.now().strftime('%Y-%m-%d-%H%M'))
    res_file_path = os.path.join(RESULTS_DIR, dataset_name, res_file_name)
...
def _mo_gaal_experiment(x_train, y_train, dataset_name, single_class_ind, gpu_q, p):
    ...
    # setup GAN
    no_G = 10  # Number of generator
    zdim = 128
    batch_size = 64
    joint_epochs = 25  # Train D and Gs jointly.
    ...
    # for data parallel
    # D = discriminator(input_dim=n_channels, output_dim=1, input_size=input_size)
    # D = torch.nn.DataParallel(D).cuda()
    # Gs = [torch.nn.DataParallel(generator(input_dim=zdim, output_dim=n_channels, input_size=input_size)).cuda() for _ in range(no_G)]
    ...
    print('Using {} generators, G iter: {}, D iter: {}.'.format(no_G, joint_epochs, D_epochs))
    ...
    for e in range(D_epochs):
        ...
        for it, (x_, _) in enumerate(trainloader):
            ...
            # TODO(demonzyj56): is this necessary? Should we update for each epoch?
    ...
    res_file_name = '{}_mo-gaal-{}_{}_{}.npz'.format(
